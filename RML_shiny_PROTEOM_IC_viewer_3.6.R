...
server <- function(input, output, session) {
  ...
  output$downloadScaterPlot <-downloadHandler(
    function() {
      filename = paste0("ScaterPlot_",input$Sample1,"_",input$Sample2,".", input$ScaterPlotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$ScaterPlotformat,
             plot=ScaterPlot(),
             dpi=600,
  ...
  output$downloadMeanScaterPlot <-downloadHandler(
    function() {
      filename = paste0("MeanScaterPlot_",input$Cond1,"_",input$Cond2,".", input$MeanScaterPlotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$MeanScaterPlotformat,
             plot=MeanScaterPlot(),
             dpi=600,
  ...
  GraphVolcano<-reactive({
    ...
    n<-if(input$ThreasholdType=="Pvalue"){0}else{1}
      ...
      y<-data.frame(Gene_ProteinGroup=x[,"Gene_ProteinGroup"])
      x<-x[,c(MyColNumber(),MyColNumber()+n,MyColNumber()+2,5)]
      x<-cbind(x,y)
